...
- **0.11.8** — two fixes for "sync looks stuck, nothing changes": (1) **no cross-destination leakage of runtime GitHub state** — the repo/credential state recorded in a backup root's `auto.json` is now discarded and re-read whenever the destination changes, so a new root can no longer push its content to the previous root's remote (found by headless e2e: an isolated home was still pointed at the real repository); (2) **live progress + cancel for rigid retries** — the panel now shows `attempt #N, elapsed Xs / window Ys, next in Zs` plus the last error while a network action retries (default window 30 min), with a **Cancel this sync** button that stops retrying without weakening the default no-degrade policy. The headless restore e2e now runs the **working tree** (staged package with the store's resolved peers) instead of the installed copy, guards against inheriting a real GitHub repo, and tolerates the host's brief live-patch-reload window.
- **0.11.3** — GitHub sync falls back to `gh` auth (personal fork `laituli/dsh-backup`): when the `https://github.com` remote has no `DSH_BACKUP_GITHUB_TOKEN` / `GITHUB_TOKEN` env token, the plugin now resolves credentials via `gh auth token` (needs `gh` installed and logged in, `gh auth login`) and reuses the existing `.git-credentials` flow — backups keep syncing without managing a PAT.
