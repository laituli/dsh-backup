...
const SETTINGS_FIELDS = ['destination', 'keep', 'exclude', 'redact', 'githubRepo', 'deployRestoreDelay'];
...
export function apply(ctx, pluginConfig) {
  ...
  async function tryGhAuthToken(cwd, signal) {
    try {
      ...
      const r = await spawnRun([gh, 'auth', 'token'], cwd, signal);
      const t = r.out.trim();
   ...
   * 只管理 dsh-* 归档与其边车——归档与边车复制进去，git add -A 同时记录
   * 轮换删除，其余文件（README.md 等说明文档）原样保留，commit 后
  ...
  async function githubSync(signal) {
    ...
    // 镜像工作树：同步只管理"备份域"文件（dsh-* 归档及其边车）。keep 集 =
    // 当前全量备份集（归档+边车）+ 凭据文件；不在 keep 的备份域文件即本地已
    // 轮换/删除的旧备份，随本次提交同步移除。仓库里的其它文件（README.md、
    // LICENSE、说明文档等）一律原样保留、绝不同步删除——冷启动指南因此可以
    // 安全地放在数据仓库根目录（v0.11.4+）；列表加载 / 滚动删除 / 轮换等机制
    // 本来就按 dsh-* 命名与 .tar.gz/边车后缀过滤，不受这些文档文件影响。
    ...
    const stale = entries
      .filter((e) => e.isFile() && MANAGED.test(e.name) && !keep.has(e.name))
      .map((e) => e.name);
