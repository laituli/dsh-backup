...
export function apply(ctx, pluginConfig) {
   ...
   * 需要“停止当前 agent 并启动新的”，这条命令正是原样重启入口，任何
   * shell 直接粘贴即可——注意：执行前需先退出正在运行的宿主（端口占用）。
   */
  function hostRelaunchCommand() {
    return [shellQuote(process.execPath), ...process.argv.slice(1).map((a) => shellQuote(a))].join(' ');
  }
   ...
   * - relaunchCmd：本进程原样启动命令，任何 shell 粘贴即可。
   */
