 ...
 * - `/backup restore <前缀|latest> --deploy [--deploy-delay N]`  部署期恢复：宿主空闲后由
 *   延时外部进程依次 停止宿主→删除(快照后挪旁)→恢复（延时缺省取设置项 deployRestoreDelay）
...
/**
 * GitHub 网络操作（fetch / push / gh auth）的有界执行上限：网络抖动或代理
 * 不可用时，「立即同步」不能无限期卡在“同步中…”（用户反馈：backup 同步
 * 按钮停留很久、结束后列表无任何变化）。超时即终止进程树并报可操作错误。
 */
const GH_NET_TIMEOUT_MS = 40 * 1000;
...
const DEFAULTS = Object.freeze({
  ...
  githubRepo: '',
  /** 部署期恢复延时秒数（deploy-restore：宿主空闲后开始倒计时，默认 60）。 */
  deployRestoreDelay: 60,
  /**
   * GitHub 网络操作超时秒数（fetch / push / gh auth / pull 共用）。真实网络
   * 常有 >5min 的 spike，故可配（5~3600）；环境变量 DSH_BACKUP_NET_TIMEOUT_SEC
   * 可临时覆盖。
   */
  githubTimeoutSec: 40,
  /** 网络动作“刚性完成”窗口（秒）：窗口内不断重试直到成功；0 = 关闭重试。 */
  githubRetryWindowSec: 1800,
  /**
   * 命令/工具出口的硬截止时间（秒）：超过即**强制结算**并把慢活留在后台。
   * DSH 按 command/run↔command/done 折叠命令卡，处理器不返回 = 界面永远
   * “进行中”且挡住后续命令（2026-09-10 真实事故）。默认 180s 只对本地归档
   * 留足余量，网络动作已全部后台化，正常路径几秒内就返回。
   */
  commandDeadlineSec: 180,
});

/**
 * withDeadline 的超时哨兵：模块级常量，闭包内外引用同一符号，避免多版本比较失效。
 */
const SYNC_DEADLINE = Symbol('dsh-backup:deadline');
...
const SCHEMA = z.object({
  ...
  deployRestoreDelay: z.number().min(0).default(DEFAULTS.deployRestoreDelay),
  githubTimeoutSec: z.number().min(0).default(DEFAULTS.githubTimeoutSec),
  githubRetryWindowSec: z.number().min(0).default(DEFAULTS.githubRetryWindowSec),
  // 命令出口的硬截止时间（配置层可调；不上面板表单，避免动面板 schema 契约）
  commandDeadlineSec: z.number().min(5).default(DEFAULTS.commandDeadlineSec),
});

const SETTINGS_FIELDS = ['destination', 'keep', 'exclude', 'redact', 'githubRepo', 'deployRestoreDelay', 'githubTimeoutSec', 'githubRetryWindowSec'];
...
function normalizeField(out, field, value) {
  ...
  } else if (field === 'deployRestoreDelay') {
    const n = Number(value);
    if (Number.isFinite(n) && n >= 0) out.deployRestoreDelay = Math.floor(n);
  } else if (field === 'githubRetryWindowSec') {
    const n = Number(value);
    if (Number.isFinite(n) && n >= 0 && n <= 86400) out.githubRetryWindowSec = Math.floor(n);
  } else if (field === 'githubTimeoutSec') {
    const n = Number(value);
    if (Number.isFinite(n) && n >= 5 && n <= 3600) out.githubTimeoutSec = Math.floor(n);
  }
}
...
const PANEL_INVOCATIONS = Object.freeze([
  ...
  panelDescriptor('restore', ['selector', 'dryRun', 'types', 'syncDeps', 'deploy', 'deployDelay'], true),
  ...
  panelDescriptor('cancelSync', []),
  panelDescriptor('doctorScan', [], true),
  panelDescriptor('doctorRepair', ['selector'], true),
  panelDescriptor('upgradePlan', [], true),
  panelDescriptor('upgradeRun', ['kind', 'name'], true),
  panelDescriptor('cancelUpgrade', [], false),
]);
...
class BackupPanelService extends TypertRemoteService {
   ...
   *  deploy 以部署期模式执行：宿主空闲后由延时外部进程 停宿主→删除(挪旁)→恢复）。 */
  restore(selector, dryRun, types, syncDeps, deploy, deployDelay, signal) {
    return this.ops.restore(selector, dryRun, types, syncDeps, deploy, deployDelay, signal);
  }
  ...
  /** 取消进行中的刚性重试（用户主动中止，不算策略降级）。 */
  cancelSync() {
    return this.ops.cancelSync();
  }

  /** 前端内升级清单：git 源依赖 + 各自当前/最新 tag（只读）。 */
  upgradePlan(signal) {
    return this.ops.upgradePlan(signal);
  }

  /** 执行升级事务（kind='all' 一键全部；否则 name 指定单个插件）。 */
  upgradeRun(kind, name, signal) {
    return this.ops.upgradeRun(kind, name, signal);
  }

  /** 取消进行中的升级事务（当前这一步结束后停止后续插件）。 */
  cancelUpgrade() {
    return this.ops.cancelUpgrade();
  }
...
export function apply(ctx, pluginConfig) {
  ...
  async function spawnRun(argv, cwd, signal, timeoutMs) {
    ...
    // 有界执行：仅对传入 timeoutMs 的调用生效（GitHub fetch/push/gh auth）。
    // 超时即终止进程树（Windows terminate 立即强杀，POSIX SIGTERM→graceMs→SIGKILL），
    // 避免慢/断网时「立即同步」无限期停在“同步中…”。
    let timedOut = false;
    const timer = timeoutMs
      ? setTimeout(() => { timedOut = true; proc.terminate?.(); }, timeoutMs)
      : null;
    try {
      ...
      if (timedOut) {
        throw new Error(`命令超过 ${Math.round(timeoutMs / 1000)} 秒未完成，已终止（远端无响应或网络不通）`);
      }
  ...
  /**
   * gh auth 回退（个人 fork）：https github.com 远端未设 env token 时，改取本机 gh
   * 登录凭据（gh auth token），免手工 PAT。token 仍只写入同步工作树内的
   * .git-credentials（credential.helper store），不进进程参数、不进提交——
   * 沿用既有凭据约束；gh 缺失/未登录时返回空串，由调用方给出 reason。
   */
  async function tryGhAuthToken(cwd, signal) {
    try {
      const gh = await ctx.subprocess.resolveExecutable('gh');
      // gh auth token 也可能触发网络/登录流程——同样有界执行，避免同步被钉住
      const r = await spawnRun([gh, 'auth', 'token'], cwd, signal, netTimeoutMs());
      const t = r.out.trim();
  ...
  function paths() {
    ...
    // 逐级兜底：launchEnvironment（服务缺失时 get 返回 undefined，如 `dsh web --help`
    // 这种没有完整宿主的装配）→ 进程环境变量 → os.homedir()。
    // 绝不 throw：0.11.9 在这里硬失败过一次，把 `dsh web --help` 打成 fatal load
    // failure（插件问题不该能打挂宿主）。
    const envHome = env?.get('HOME')?.value || env?.get('USERPROFILE')?.value;
    const homeRaw = envHome || process.env.HOME || process.env.USERPROFILE
      || (typeof os.homedir === 'function' ? os.homedir() : undefined);
    const home = homeRaw ? toFwd(String(homeRaw).replace(/\/+$/, '')) : undefined;
    const dshHome = env?.get('DSH_HOME')?.value || process.env.DSH_HOME || (home ? `${home}/.dsh` : undefined);
    if (!home || !dshHome) throw new Error('无法解析 HOME/USERPROFILE 或 DSH_HOME（launchEnvironment 缺失，且环境变量与 os.homedir() 都不可用）');
  ...
  async function doBackup(keep, signal, opts = {}) {
    ...
    try {
      ...
      // fork 增量的运维入口：与上游救援通道并列落盘，灾时/重启都能就地调用
      await copyFile(new URL('../rescue/ops.mjs', import.meta.url), `${root}/ops.mjs`);
    ...
    // GitHub 同步：默认**后台化**——命令/工具/面板一律不等网络，秒级结算。
    // 只有显式 opts.sync === 'await' 才同步等待（调用方自己承担等待，且仍受
    // 命令出口的硬截止时间护栏保护）。失败不回滚备份；结论记入 auto.json。
    let sync = null;
    if (opts?.sync === 'await') {
      try {
        sync = await withWorktreeLock(() => githubSync(signal));
        await applySyncOutcome(sync, null);
      } catch (err) {
        const message = String(err && err.message ? err.message : err);
        sync = { error: message };
        await applySyncOutcome(null, message);
      }
    } else {
      const started = startBackgroundSync(opts?.syncReason ?? 'backup', signal);
      // 已有后台任务在跑时复用（不并发第二份 git），快照如实反映当前状态
      sync = { background: true, ...(started.snapshot ?? {}) };
    }
  ...
  async function ensureSyncWorktree(signal) {
    ...
    // gh auth 回退（个人 fork）：https github.com 远端未设 env token 时，改取本机 gh
    // 登录凭据；token 注入 cfg 后，后续凭据落盘/镜像 keep 沿用既有 cfg.token 分支。
    if (!cfg.token && cfg.repo.startsWith('https://github.com')) {
      cfg.token = await tryGhAuthToken(root, signal);
    }
    if (cfg.repo.startsWith('https://') && !cfg.token) {
      return { reason: 'https 远端缺少 GitHub 凭据：环境变量 DSH_BACKUP_GITHUB_TOKEN / GITHUB_TOKEN 未设置，且 gh 不可用或未登录（gh auth login）' };
    }
    ...
    if (remotes.out.includes('origin')) {
      // 幂等：地址可能已被改过（面板改仓库 / 上次初始化中途失败）。两条并发路径
      // （后台同步任务 + 面板拉取）在同一工作树里各跑一次 `remote add` 会以
      // "remote origin already exists" 直接失败——set-url 把它变成可重入操作。
      await spawnRun([git, 'remote', 'set-url', 'origin', cfg.repo], syncDir, signal);
    } else {
  ...
  /** 读取 ref（HEAD / origin/main 等）跟踪的顶层文件名；ref 不存在（空仓/未提交）返回空数组。 */
  async function trackedNames(git, syncDir, ref, signal) {
    try {
      const r = await spawnRun([git, 'ls-tree', '-r', '--name-only', ref], syncDir, signal);
   ...
   * 只管理 dsh-* 归档与其边车，README.md / LICENSE 等说明文档原样保留，
   * commit 后 `push HEAD:main --force-with-lease`。https 远端的 token 只写入
   * 工作树内的 .git-credentials（credential helper），不进进程参数。
   *
   * 同步语义（修复「立即同步卡住 / 点完没反应 / 误删其它机器归档」）：
   * - 无本地变更（既没有新归档要推、也没有本机轮换待同步）时**不访问网络**，
   *   立即返回「无变更」——不再无条件先 git fetch，慢/断网时按钮不再长时间
   *   停在“同步中…”。
   * - 有变更才联网，fetch / push 均受 GH_NET_TIMEOUT_MS 有界执行，超时即终止。
   * - 删除只作用于本机**上一轮推送过的备份**（镜像 HEAD 基线内、且已不在本机
   *   keep 集）——本地轮换/删除照常同步移除。远端上本机从未推送过、也尚未
   *   拉取的归档（其它机器推的）**绝不删除**：检测到这类文件时本次同步先停下、
   *   提示先「从 GitHub 拉取」把远端归档取回本机再推——防止落后的机器把
   *   多机共享仓库里其它机器刚推的归档当 stale 覆盖掉（数据丢失）。
   */
  async function githubSync(signal) {
    netCancelRequested = false; // 新一轮同步：清除上一次的取消请求
    await adoptRuntimeStateRoot(); // 目的地切换后不得沿用旧根的 GitHub 配置
    const { root } = paths();
    const wt = await ensureSyncWorktree(signal);
    if (wt.reason) return { skipped: wt.reason };
    const { syncDir, git } = wt;

    const MANAGED = /^dsh-[\w.-]+\.(tar\.gz|sha256|meta\.json|redacted\.json)$/;
    const topManaged = (names) => names.filter((n) => !n.includes('/') && MANAGED.test(n));
    // 基线 = 本机上一轮成功推送到远端的备份域文件（镜像 HEAD 内容）；
    // 尚无本地提交（首次同步）时为空。pull 会先把镜像 reset 到远端，因此
    // 拉取过的归档也自然进入基线，后续轮换删除可继续传播。
    const baseline = new Set(topManaged(await trackedNames(git, syncDir, 'HEAD', signal)));
    // keep 集 = 本机当前备份域文件（listBackups 已排除分类型 dsh-t-* 与内部
    // 快照，故类型归档不进 GitHub 同步——含 credentials 明文者尤甚）。
    ...
    let dirty = false;
    for (const f of keep) {
      if (MANAGED.test(f) && !present.has(f) && (await existsStat(`${root}/${f}`))) { dirty = true; break; }
    }
    if (!dirty) {
      for (const f of baseline) {
        if (!keep.has(f)) { dirty = true; break; }
      }
    }
    if (!dirty) {
      // 快路径也要收拾镜像：本地没有新归档不联网，但工作树里凭空多出的杂物
      // 绝不能留着——下一次真实同步的 `git add -A` 会把它一起推上远端。
      const stray = await cleanStrayWorktreeFiles(git, syncDir, keep, signal);
      return { skipped: '无变更', ...(stray.length ? { cleaned: stray } : {}) };
    }

    // ---- 有变更：先对齐远端（有界网络操作）----
    try {
      await runNetworkStrict('git-fetch', () => spawnRun([git, ...gitSlowLinkArgs(), 'fetch', 'origin'], syncDir, signal, netTimeoutMs()), signal);
    } catch (err) {
      if (signal?.aborted) throw err;
      return { error: networkReason(String(err && err.message ? err.message : err)) };
    }
    let remoteMain = false;
    try {
      const rp = await spawnRun([git, 'rev-parse', '--verify', '-q', 'origin/main'], syncDir, signal);
      remoteMain = Boolean(rp.out.trim());
    } catch (err) {
      if (signal?.aborted) throw err;
      remoteMain = false; // 远端尚无 main（空仓 / 首次推送）
    }

    if (remoteMain) {
      // 远端存在、但本机 keep 与基线里都没有的归档 = 其它机器推送、本机尚未
      // 拉取。删除它们就是数据丢失：本次先停，指引用户拉取后再推（多机护栏）。
      const remoteFiles = topManaged(await trackedNames(git, syncDir, 'origin/main', signal));
      const foreignArchives = [...new Set(remoteFiles
        .filter((n) => !keep.has(n) && !baseline.has(n))
        .map((n) => n.replace(/\.(sha256|meta\.json|redacted\.json)$/, ''))
      )].sort();
      if (foreignArchives.length) {
        const shown = foreignArchives.slice(0, 3).join('、');
        const more = foreignArchives.length > 3 ? ` 等 ${foreignArchives.length} 份` : '';
        return {
          skipped: `远端有 ${foreignArchives.length} 份本机尚未拉取的备份（${shown}${more}）。为避免误删其它机器推送的归档，本次未推送——请先点「从 GitHub 拉取」把远端备份取回本机，再点「立即同步」`,
          blocked: true,
        };
      }
      // 对齐远端后叠加本地备份集；说明文档等非 dsh-* 文件随之保留
    ...
    // 删除只限「基线内已轮换」的备份域文件（基线 ∩ ¬keep）；远端其它机器的
    // 归档不在基线、已在上方拦截，说明文档等非备份域文件永不删除。
    const stale = entries
      .filter((e) => e.isFile() && MANAGED.test(e.name) && baseline.has(e.name) && !keep.has(e.name))
      .map((e) => e.name);
    if (stale.length) await removeFiles(stale, syncDir, signal);

    await cleanStrayWorktreeFiles(git, syncDir, keep, signal);
    ...
    try {
      await runNetworkStrict('git-push', () => spawnRun([git, ...gitSlowLinkArgs(), 'push', 'origin', 'HEAD:main', '--force-with-lease'], syncDir, signal, netTimeoutMs()), signal);
    } catch (err) {
      if (signal?.aborted) throw err;
      const raw = String(err && err.message ? err.message : err);
      // force-with-lease 被拒（远端在我们 fetch 后被推进）等——给可操作提示而非英文堆栈
      if (/\[rejected\]|stale info|fetch first|non-fast-forward|failed to push/i.test(raw)) {
        return { error: `远端在同步期间被其它推送更新，push 被 --force-with-lease 拒绝（保护生效）。请再点一次「立即同步」重试；若持续失败，先「从 GitHub 拉取」对齐远端` };
      }
      return { error: networkReason(raw) };
    }
    return { pushed: true, at: new Date().toISOString(), tooBig };
  }

  /**
   * 清理同步工作树里的"异物"：镜像只承载备份域文件 + 说明/凭据配置。
   * 只删 **未跟踪**（git status `??`）且不在 keep/白名单里的条目——已跟踪的文件
   * （可能是别的机器推上来的说明文档）永不删除，避免把别人的东西顺手删掉。
   * 返回被清理的名字；任何 git 失败都静默返回空（清理是加固，不是主流程）。
   */
  async function cleanStrayWorktreeFiles(git, syncDir, keep, signal) {
    const ALWAYS = new Set(['.gitignore', '.git-credentials', 'README.md']);
    try {
      const st = await spawnRun([git, 'status', '--porcelain', '--untracked-files=all'], syncDir, signal);
      const stray = st.out.split('\n')
        .filter((line) => line.startsWith('?? '))
        .map((line) => line.slice(3).trim().replace(/^"(.*)"$/, '$1'))
        .filter((name) => name && !keep.has(name) && !ALWAYS.has(name));
      if (stray.length) await removeFiles(stray, syncDir, signal);
      return stray;
    } catch {
      return [];
    }
  ...
  function summarizeSync(s) {
    ...
    if (s.blocked) return `⚠️ 同步未执行：${s.skipped || '远端状态需要先处理'}`;
    if (s.pushed) return `✅ GitHub 同步完成: ${s.at}${s.tooBig?.length ? `\n跳过超大文件: ${s.tooBig.join(', ')}` : ''}`;
    return `GitHub 同步: ${s.skipped || '无变更'}${s.tooBig?.length ? `\n跳过超大文件: ${s.tooBig.join(', ')}` : ''}`;
  }

  /**
   * 生效的 GitHub 网络操作超时（毫秒）：环境变量 DSH_BACKUP_NET_TIMEOUT_SEC >
   * 设置项 githubTimeoutSec > 内置默认。真实网络常见 >5min 的 spike，故上限
   * 放到 3600s；下限 5s 防呆。
   */
  function netTimeoutMs() {
    const env = Number(process.env.DSH_BACKUP_NET_TIMEOUT_SEC);
    const cfgSec = Number(resolvedConfig()?.githubTimeoutSec);
    const sec = Number.isFinite(env) && env >= 5
      ? env
      : (Number.isFinite(cfgSec) && cfgSec >= 5 ? cfgSec : GH_NET_TIMEOUT_MS / 1000);
    return Math.min(3600, Math.max(5, Math.floor(sec))) * 1000;
  }
   ...
   */
  /**
   * 网络类错误的统一人话翻译：git 的 "Failed to connect ... Timed out /
   * Could not resolve host / unable to access" 这类英文原文对用户毫无指导意义，
   * 统一换成「无法连接 GitHub（网络或代理不可达）+ 技术细节 + 下一步动作」。
   */
  function networkReason(raw) {
    const text = String(raw ?? '');
    if (/Could not resolve host|Failed to connect|Connection (refused|reset|timed out)|Timed out|unable to access|Operation timed out|SSL|proxy|schannel/i.test(text)) {
      return `无法连接 GitHub（网络或代理不可达）：${snippet(text, 160)}。请检查代理/VPN 或稍后重试；网络恢复后再点一次即可`;
    }
    if (/命令超过 \d+ 秒未完成/.test(text)) {
      return `${text}。请检查代理/VPN 后重试`;
    }
    return friendlyReason(new Error(text));
  }

  async function githubPull(signal) {
    netCancelRequested = false; // 新一轮拉取：清除上一次的取消请求
    await adoptRuntimeStateRoot(); // 目的地切换后不得沿用旧根的 GitHub 配置
    const { root, home } = paths();
    const wt = await ensureSyncWorktree(signal);
    if (wt.reason) throw new Error(wt.reason);
    const { syncDir, git } = wt;
    // 与同步一致：网络操作有界执行 + 失败给人话（否则 21s 后甩一条英文 fatal）
    try {
      await runNetworkStrict('git-pull-fetch', () => spawnRun([git, ...gitSlowLinkArgs(), 'fetch', 'origin', 'main'], syncDir, signal, netTimeoutMs()), signal);
    } catch (err) {
      if (signal?.aborted) throw err;
      throw new Error(networkReason(String(err && err.message ? err.message : err)));
    }
    await spawnRun([git, 'reset', '--hard', 'origin/main'], syncDir, signal);
  ...
  async function restoreArchive(selector, dryRun, signal, opts = {}) {
    const { home, dshHome, root } = paths();
    // 备份目录落在数据目录内部时，整包恢复先把 dshHome 挪旁，归档路径随即失效
    // （解压 ENOENT）。与 rescue 同语义提前拦住，给出可操作指引。
    if (root === dshHome || root.startsWith(`${dshHome}/`)) {
      throw new Error(`备份目录（${root}）位于数据目录（${dshHome}）内部：整包恢复会把它一起挪走，归档随即不可读。请把备份 destination 指到数据目录之外（如 ~/Desktop/dsh-backups）后重试。`);
    }
  ...
  // ---------- 悬挂命令封口（session log 追加式修复） ----------
  //
  // 事故背景（2026-09-10，真实）：宿主在 `/backup` 内联等待 `git push` 时被强杀，
  // 日志里只留下 `command/run`（没有配对的 `command/done`）。DSH 客户端按
  // command/run↔command/done 折叠命令卡——没有 done 就**永远显示「进行中」**，
  // 而且该会话的后续命令/消息一起被挡住。宿主本身不会自愈（内存里的状态一直
  // 是"在跑"），只能把日志补齐。
  //
  // 修复方式刻意做成**追加一帧**而不是重写整份日志：宿主读端契约要求首帧恰好
  // 一行 SessionHeader（assertZstdHeaderFrame），单帧重写会被整体拒载；追加帧
  // 天然满足契约，seq 只要严格接续即可。

  /** 扫描 log 行，解析出 seq 游标、悬挂的 command/run、悬挂的 tool/call。 */
  function scanDangling(lines) {
    ...
    for (let i = 1; i < lines.length; i++) {
      let rec;
      try { rec = JSON.parse(lines[i]); } catch { continue; } // 坏行由 validateSessionLines 另行报错
      ...
      if (rec.type === 'command/run' && rec.data?.commandId) {
        runs.set(rec.data.commandId, { name: String(rec.data.name ?? '?'), seq: rec.seq ?? null, time: rec.time ?? null });
      } else if (rec.type === 'command/done' && rec.data?.commandId) {
        dones.add(rec.data.commandId);
      }
    }
    const dangling = [...runs.entries()].filter(([id]) => !dones.has(id)).map(([id, info]) => ({ commandId: id, ...info }));
    return { nextSeq, lastTime, dangling };
  }

  /**
   * 给指定会话日志补一条 `command/done`（kind: error）把悬挂的命令配对了结。
   * 追加一帧 + 原子替换；原文件先留档 `*.preseal-<stamp>`，任何一步失败都保持原样。
   */
  async function sealSessionLog(abs, rel, opts = {}) {
    ...
    const records = dangling.map((d, i) => JSON.stringify({
      ...
      data: {
        ...
        kind: 'error',
        text: `已中断：宿主在 /${d.name === 'backup' ? 'backup' : d.name} 执行期间被停止（多半是同步卡住时强制重启）。这张卡片在界面上会一直显示「进行中」——此记录把它按「已中断」结清。可重新执行该命令。`,
      },
    }));
    if (opts.dryRun) return { rel, sealed: dangling.map((d) => d.commandId), dryRun: true, nextSeq };

    const stamp = stampNow();
    const backup = `${abs}.preseal-${stamp}`;
    await copyFile(abs, backup);
    const appended = Buffer.from(`${records.join('\n')}\n`, 'utf8');
    const out = isZstd
      ? Buffer.concat([raw, zlib.zstdCompressSync(appended)])
      : Buffer.concat([raw, appended]);
    ...
    try {
      ...
      throw new Error(`封口写入失败（${err && err.code ? err.code : String(err)}）：宿主可能仍持有该日志（先停止 dsh 再试）。原文件未改动`);
    }
    // 复检：封口后必须仍然可解码、seq 连续
    const verdict = await validateSessionFile({ abs, rel });
    return {
      rel,
      sealed: dangling.map((d) => `${d.commandId}(${d.name})`),
      backup,
      nextSeq,
      verified: verdict.state,
      ...(verdict.state === 'ok' ? {} : { verifyReason: verdict.reason }),
    };
  }

  /** 封口扫描：默认只看最近 14 天内改动过的日志；改动 <120s 的视为"运行中"跳过。 */
  async function sealDanglingCommands(opts = {}) {
    const { dshHome } = paths();
    const files = [];
    await collectSessionLogs(dshHome, '', 0, files);
    const now = Date.now();
    const maxAgeMs = (opts.days ?? 14) * 86400000;
    const activeMs = (opts.activeMs ?? 120000);
    const results = [];
    const skipped = [];
    for (const f of files) {
      let info;
      try { info = await fsStat(f.abs); } catch { continue; }
      if (info.size === 0) continue;
      if (now - info.mtimeMs > maxAgeMs) { skipped.push({ rel: f.rel, why: `超过 ${opts.days ?? 14} 天未改动` }); continue; }
      if (opts.force !== true && now - info.mtimeMs < activeMs) {
        // 正在写的会话（宿主在跑）：此刻追加会与宿主抢文件（Windows 上 rename 直接
        // 失败），且宿主内存状态也不会因日志变化而刷新——先停 dsh 再封口。
        skipped.push({ rel: f.rel, why: '最近 2 分钟仍在写入（会话可能正被宿主使用；先停止 dsh）' });
        continue;
      }
      try {
        ...
        skipped.push({ rel: f.rel, why: String(err && err.message ? err.message : err) });
      }
    }
    return { scanned: files.length, results, skipped };
  }

  function summarizeSeal(r, opts = {}) {
    if (!r.results.length) {
      const tail = r.skipped.length ? `\n（跳过 ${r.skipped.length} 个：${r.skipped.slice(0, 3).map((s) => `${s.rel} — ${s.why}`).join('；')}${r.skipped.length > 3 ? ' …' : ''}）` : '';
      return `✅ 未发现「只 run 没有 done」的悬挂命令（扫描 ${r.scanned} 个会话日志）${tail}`;
    }
    const lines = [`🩹 ${opts.dryRun ? '（预览）' : ''}已封口 ${r.results.length} 个会话日志里的悬挂命令：`];
    for (const x of r.results) {
      lines.push(`  ${x.rel}`);
      lines.push(`    命令: ${x.sealed.join(', ')}${x.backup ? `\n    原日志留档: ${x.backup}` : ''}${x.verified ? `\n    复检: ${x.verified === 'ok' ? '健康' : x.verifyReason}` : ''}`);
    }
    if (r.skipped.length) lines.push(`  跳过 ${r.skipped.length} 个（${r.skipped.slice(0, 2).map((s) => s.why).join('；')}${r.skipped.length > 2 ? ' …' : ''}）`);
    lines.push('  这些命令卡会从「进行中」变为「已中断」——**重启 dsh 后生效**（宿主内存态不会因为日志变化而刷新）。');
    return lines.join('\n');
  }
  ...
  //
  // 0.11.13 追加**进程内串行化**：后台同步任务也会落 state，两次 rename 撞同一
  // 目标路径在 Windows 上直接 EPERM（实测：smoke 第 1 次备份因 auto.json 改名
  // 失败）。前一个写完再发下一个，跨实例的原子性仍由 UUID 临时名保证。
  let autoStateWrite = Promise.resolve();
  function saveAutoState() {
    const next = autoStateWrite.then(() => saveAutoStateNow(), () => saveAutoStateNow());
    autoStateWrite = next.then(() => {}, () => {});
    return next;
  }

  async function saveAutoStateNow() {
    const { root } = paths();
    const target = `${root}/auto.json`;
    // 临时名用 UUID：时间戳+实例内序号在"多实例同毫秒启动"时仍会撞名
    // （先到者 rename 走，后到者 ENOENT），UUID 跨实例唯一
    const tmp = `${target}.${randomUUID()}.tmp`;
    const payload = `${JSON.stringify({ hours: autoHours, lastAutoAt, github: githubState, lastTrain })}\n`;
    // Windows 上「目标文件此刻正被别人打开」会让 rename 抛 EPERM/EACCES/EBUSY——
    // 跨进程（多实例/多 profile 共用备份目录）撞车时是**暂时性**失败，重试即可；
    // 不重试会造成"备份明明做完了却报权限不足"的假失败（实测）。
    const transient = /^(EPERM|EACCES|EBUSY|EEXIST)$/;
    for (let attempt = 1; ; attempt += 1) {
      try {
        await writeOwned(tmp, payload);
        await rename(tmp, target);
        return;
      } catch (err) {
        await rm(tmp, { force: true }).catch(() => {});
        if (attempt >= 5 || !transient.test(String(err && err.code))) throw err;
        await new Promise((r) => setTimeout(r, 40 * attempt));
      }
    }
  }

  async function loadAutoState() {    try {
      const { root } = paths();
  ...
  /**
   * 运行时状态（auto.json）当前归属的备份根。
   * 目的地变更后必须重读：旧的 GitHub 仓库/凭据状态若被沿用到新根，就会把新根的
   * 内容推到旧根配置的远端（实测踩到过：隔离验证环境把临时 home 往用户真实仓库推）。
   */
  let runtimeStateRoot = null;

  /**
   * 目的地（备份根）变了 → 作废旧根的运行时 GitHub 状态，并从新根重读 auto.json。
   * 首次调用只记录归属（启动时 loadAutoState 已按当时的 root 读过）。
   */
  async function adoptRuntimeStateRoot() {
    const { root } = paths();
    if (runtimeStateRoot === null) { runtimeStateRoot = root; return; }
    if (runtimeStateRoot === root) return;
    const prev = runtimeStateRoot;
    runtimeStateRoot = root;
    githubState = { repo: null, lastPush: null, lastError: null };
    lastTrain = null;
    await loadAutoState();
    console.warn(`[dsh-backup] 备份目的地已切换（${prev} → ${root}）：已重读该根的 auto.json，上一个根的 GitHub 同步仓库不再沿用（要在新根继续同步，请在面板重新填写仓库）。`);
  }
  ...
  // ---------- /backup 命令 ----------
  /**
   * /backup 的实体（内部函数）：**每个出口都返回值、绝不悬挂**。外层 handler 再套
   * 一层硬截止时间护栏——两层保证命令卡不会永远停在「进行中」（DSH 按
   * command/run↔command/done 折叠，缺 done 就等于永久进行中，还会挡住后续命令）。
   */
  async function runBackupCommand(invocation) {
      ...
      try {
        ...
        if (head === 'restore') {
          ...
          const deploy = parts.includes('--deploy');
          ...
          if (deploy && !dryRun) {
            const picked = await pickArchive(sel).catch(() => null);
            const delayMatch = input.match(/--deploy-delay\s+(\d+)/);
            const arm = armDeployRestore({ selector: sel, delay: delayMatch ? Number(delayMatch[1]) : undefined, archiveName: picked ? picked.name : undefined });
            return { kind: 'success', text: arm.summary };
          }
        ...
        if (head === 'doctor') {
          // --seal：给「只 run 没有 done」的悬挂命令补一条 done（命令卡不再永远
          // 显示「进行中」）；--dry-run 只预览。会话日志正在被宿主使用时会跳过。
          if (parts.includes('--seal') || parts.includes('--unstick')) {
            const dryRun = parts.includes('--dry-run');
            // --force：连"最近 2 分钟仍在写入"的日志也封（宿主已确证停掉时用；
            // 部署脚本里停机与封口是连着的，这个开关让流程不必"等两分钟再跑一次"）
            const force = parts.includes('--force');
            const r = await sealDanglingCommands({ dryRun, force });
            return { kind: 'success', text: summarizeSeal(r, { dryRun }) };
          }
        ...
        if (head === 'github') {
          ...
          if (arg === 'sync') {
            // 默认后台（秒级结算）；--wait 才同步等待——用户明确要结果时才等，
            // 且仍受命令出口的硬截止时间护栏保护。
            if (parts.includes('--wait')) {
              const s = await withWorktreeLock(() => githubSync(invocation.signal));
              await applySyncOutcome(s, null);
              return { kind: s.blocked ? 'error' : 'success', text: summarizeSync(s) };
            }
            const started = startBackgroundSync('github sync', invocation.signal);
            return {
              kind: 'success',
              text: started.started
                ? `🚀 已在后台开始 GitHub 同步（不阻塞聊天）。\n  ${syncJobLine()}`
                : `ℹ️ 已有一个 GitHub 同步任务在跑，本次不重复发起（同一工作树不并发 git）。\n  ${syncJobLine()}`,
            };
          }
          if (arg === 'cancel') {
            if (!netProgress && (!syncJob || syncJob.settled)) {
              return { kind: 'error', text: `当前没有进行中的网络动作。\n  ${syncJobLine()}` };
            }
            netCancelRequested = true;
            return { kind: 'success', text: `🛑 已请求取消「${netProgress ? netProgress.label : syncJob?.reason ?? 'sync'}」：当前这次尝试结束后立即停止重试（最多再等 ${Math.round(netTimeoutMs() / 1000)}s）。\n  ${syncJobLine()}` };
          }
          if (arg === 'pull') {
            ...
            const p = await withWorktreeLock(() => githubPull(invocation.signal));
            const lines = [];
            if (p.pulled.length) {
          ...
          if (arg === 'status') {
            const cfg = githubConfig();
            const text = cfg
              ? `GitHub 同步: ${cfg.repo}\n  token: ${cfg.token ? '已配置' : '未配置（https 远端需要）'}\n  ${syncJobLine()}\n  ${githubState.lastPush ? `上次成功推送: ${githubState.lastPush}` : '尚未推送过'}\n  ${githubState.lastError ? `上次错误: ${githubState.lastError}` : ''}\n  /backup github repo <地址> 可修改；/backup github cancel 取消进行中的同步`
              : 'GitHub 同步未配置：/backup github repo <owner/repo> 设置，或在 cordis.yml 的 config.githubRepo 配置。';
            return { kind: 'success', text };
          }
          return { kind: 'error', text: '用法: /backup github status|sync [--wait]|cancel|pull [--restore <前缀|latest>]|repo <地址|off>' };
        }

        // 顶层便捷别名：/backup sync（后台推送）、/backup cancel（取消网络动作）
        if (head === 'sync') {
          const started = startBackgroundSync('backup sync', invocation.signal);
          return {
            kind: 'success',
            text: started.started
              ? `🚀 已在后台开始 GitHub 同步（不阻塞聊天）。\n  ${syncJobLine()}`
              : `ℹ️ 已有一个 GitHub 同步任务在跑，本次不重复发起。\n  ${syncJobLine()}`,
          };
        }
        if (head === 'cancel') {
          if (!netProgress) return { kind: 'error', text: `当前没有进行中的网络动作。\n  ${syncJobLine()}` };
          netCancelRequested = true;
          return { kind: 'success', text: `🛑 已请求取消「${netProgress.label}」（最多再等 ${Math.round(netTimeoutMs() / 1000)}s 生效）。` };
        }
  ...
  ctx.commands.register({
    name: 'backup',
    description: '备份/恢复 DSH 数据；子命令: list | verify [前缀|all] | restore <前缀|latest> [--dry-run] [--sync-deps] [--deploy] [--deploy-delay N] | doctor [--repair <前缀|latest>] | auto [N小时|off] | github status|sync|cancel|pull|repo | sync | [--keep N]',
    /**
     * 硬截止时间护栏：内部无论哪一步卡住，命令都必须结算。超时不取消底层工作
     * （归档/推送会继续跑完并落在状态与面板里），但命令卡立刻收敛为「已超时」，
     * 后续命令/聊天消息不再被堵住。
     */
    handler: async (invocation) => {
      const limit = commandDeadlineMs();
      const raced = await withDeadline(runBackupCommand(invocation), limit, '/backup');
      if (raced.timedOut) {
        return {
          kind: 'error',
          text: `⏱️ /backup 超过 ${Math.round(limit / 1000)}s 未完成，已强制结束本次调用（慢活仍在后台继续，数据不会因此损坏）。\n  ${syncJobLine()}\n  查结果: /backup list、/backup github status；网络类动作可 /backup github cancel 取消`,
        };
      }
      if (raced.error) return { kind: 'error', text: `备份失败: ${friendlyReason(raced.error)}` };
      return raced.value;
    },
  });
  ...
  ctx.tools.register(defineTool({
    name: 'backup_dsh',
    description: '备份、校验、恢复或体检 DSH 用户数据（~/.dsh 的会话、配置、技能）。凭据文件默认脱敏：不进归档、不进 GitHub 同步，明文只存本机 vault，恢复时自动还原。mode=backup 立即备份（keep 指定保留份数）；mode=list 列出备份；mode=verify 校验完整性（selector=前缀或 all，缺省最新一份）；mode=restore 恢复（selector=前缀或 latest，syncDeps 恢复后重装 profile 依赖；恢复前自动校验并快照当前数据，解压失败自动还原；真实恢复前必须先 dryRun=true 预览并向用户展示确认；deploy=true 时以部署期模式执行——宿主确认无在途聊天轮次后，由延时外部进程依次 停止宿主→删除(快照后挪旁)→恢复，本调用立即返回凭据，实际恢复发生在宿主空闲+延时之后，可删中止文件取消）；mode=auto 设置定时备份（hours 间隔小时数，0=关闭，缺省查询）；mode=doctor 会话日志体检——检测损坏的会话历史（seq 撞号/坏帧/截断），repair=true 时从 selector 归档定点修复',
    parameters: {
      ...
      deploy: { type: 'boolean', description: 'mode=restore 且 dryRun=false 时以部署期模式执行：宿主空闲后由延时外部进程 停止宿主→删除(挪旁)→恢复；立即返回凭据，实际恢复延后发生（可删返回的中止文件取消）' },
      deployDelay: { type: 'number', description: 'mode=restore 且 deploy=true 时覆盖部署期恢复延时秒数（缺省取设置项 deployRestoreDelay=60）' },
    ...
    execute: async (args, exec) => {
      ...
      try {
        ...
        if (mode === 'restore') {
          ...
          if (args && args.deploy && !dryRun) {
            const picked = await pickArchive(selector || 'latest').catch(() => null);
            const arm = armDeployRestore({ selector: selector || 'latest', delay: args && args.deployDelay, archiveName: picked ? picked.name : undefined });
            return { ok: true, ...arm };
          }
          const r = await restoreArchive(selector || 'latest', dryRun, signal, { syncDeps: Boolean(args && args.syncDeps), ...(types ? { types } : {}) });
        ...
        return { ok: true, path: r.path, sha: r.sha, sync: syncJobSnapshot(), summary: `备份完成: ${r.path}${types ? `（类型: ${types.join(', ')}${r.hasCredentials ? '，含明文凭据·不进 GitHub 同步' : ''}）` : ''}\nsha256: ${r.sha}\n轮换删除 ${r.stale} 份（保留 ${r.keep} 份）${quarantineNote(r)}\n${syncJobLine()}` };
      } catch (err) {
  ...
  // ---------- 部署期恢复（deploy-restore：武装→空闲信号→延时外部进程→停宿主→删除(挪旁)→恢复） ----------

  /** 生效的部署期恢复延时秒数：入参 > 设置项 deployRestoreDelay > 默认 60。 */
  function deployRestoreDelaySec(override) {
    const n = Number(override);
    if (Number.isFinite(n) && n >= 0) return Math.floor(n);
    const cfg = resolvedConfig();
    const m = Number(cfg && cfg.deployRestoreDelay);
    return Number.isFinite(m) && m >= 0 ? Math.floor(m) : DEFAULTS.deployRestoreDelay;
  }

  /**
   * 刚性重试的实时进度（面板在长时间等待里显示“还活着”的唯一数据源）：
   * null = 当前没有网络动作在跑。
   */
  let netProgress = null;
  /**
   * 面板「取消本次同步」置位：用户主动中止，不算“因为网络波动降级”——
   * 每次用户/自动发起的同步入口都会重置它（见 githubSync / githubPull 起点）。
   */
  let netCancelRequested = false;

  /**
   * 网络同步的「后台任务」状态机（0.11.13 起）：命令/工具/面板一律不再**等待**
   * 网络，只发起任务并立刻结算。
   *
   * 为什么必须这样（真实事故，2026-09-10）：`/backup` 在 v0.11.4 里内联
   * `await githubSync()`，而那次 `git push` 既没有单次超时、也没有后台化——
   * 命令处理器一直不返回，宿主日志里只剩一条孤零零的 `command/run`（没有配对的
   * `command/done`）。DSH 客户端按 `command/run`↔`command/done` 折叠命令卡，没有
   * done 就永远显示「备份进行中」；该会话/压缩命令也一起被挡住（用户报「无法调度
   * /compact」「上发聊天没有聊天反馈」）。结论：命令必须**秒级结算**，慢活转后台。
   *
   * 状态口径：null=从未跑过；settled=false=进行中（面板/命令据此显示进度）；
   * settled=true=已结束（result 或 error 二选一，永久保留最近一次）。
   */
  let syncJob = null;
  /** 同步互斥：同一进程内绝不并发两份 git 操作（并发会在同一工作树上抢 ref 锁）。 */
  let syncInFlight = null;
  /**
   * 工作树全局串行锁：**所有**碰 .github-sync 的流程（后台同步任务、面板拉取、
   * 命令面 --wait 同步）都从这里过。并发下 git 的分支/远端/索引操作会互相打架
   * （实测：两路初始化同跑 → "remote origin already exists"），串行化是最省心的
   * 正确性保证；代价只是"慢一点"，而每步都有网络超时兜底。
   */
  let worktreeQueue = Promise.resolve();
  function withWorktreeLock(fn) {
    const next = worktreeQueue.then(fn, fn);
    worktreeQueue = next.then(() => {}, () => {});
    return next;
  }

  /** 后台任务快照（纯值，可直接进 RPC 回执/命令文本）。 */
  function syncJobSnapshot() {
    if (!syncJob) return null;
    return {
      state: syncJob.settled ? (syncJob.error ? 'error' : 'done') : 'running',
      reason: syncJob.reason,
      startedAt: new Date(syncJob.startedAt).toISOString(),
      elapsedMs: (syncJob.finishedAt ?? Date.now()) - syncJob.startedAt,
      finishedAt: syncJob.finishedAt ? new Date(syncJob.finishedAt).toISOString() : null,
      result: syncJob.result ?? null,
      error: syncJob.error ?? null,
    };
  }

  /** 后台任务的一行中文摘要（命令回执/面板都用它，避免两处口径漂移）。 */
  function syncJobLine() {
    const s = syncJobSnapshot();
    if (!s) return 'GitHub 同步：未运行';
    const secs = Math.round(s.elapsedMs / 1000);
    if (s.state === 'running') {
      const attempt = netProgress ? `（第 ${netProgress.attempt + 1} 次尝试、已用 ${secs}s、窗口 ${Math.round((netProgress.windowMs ?? 0) / 1000)}s）` : '';
      return `GitHub 同步：后台进行中${attempt}——/backup github status 看进度，/backup github cancel 取消`;
    }
    if (s.state === 'error') return `GitHub 同步：后台任务失败（用时 ${secs}s）—— ${s.error}`;
    return `${summarizeSync(s.result ?? {})}（后台，用时 ${secs}s）`;
  }

  /** 同步结果落到持久状态（auto.json 的 github.lastPush/lastError）。 */
  async function applySyncOutcome(result, error) {
    if (error) {
      githubState = { ...githubState, lastError: String(error) };
    } else if (result?.pushed) {
      githubState = { ...githubState, lastPush: result.at, lastError: null };
    } else if (result?.error) {
      githubState = { ...githubState, lastError: result.error };
    }
    try {
      await saveAutoState();
    } catch { /* 状态落盘失败不影响本次同步结论 */ }
  }

  /**
   * 发起后台 GitHub 同步（幂等：已有任务在跑时直接复用，不再并发第二份 git）。
   * 立即返回快照，绝不抛出——调用方（命令/工具/面板）据此秒级结算。
   */
  function startBackgroundSync(reason, signal) {
    if (syncInFlight) return { started: false, snapshot: syncJobSnapshot() };
    netCancelRequested = false;
    const job = { reason, startedAt: Date.now(), finishedAt: null, settled: false, result: null, error: null };
    syncJob = job;
    syncInFlight = (async () => {
      try {
        const s = await withWorktreeLock(() => githubSync(signal));
        job.result = s;
        await applySyncOutcome(s, null);
      } catch (err) {
        job.error = networkReason(String(err && err.message ? err.message : err));
        await applySyncOutcome(null, job.error);
        console.warn(`[dsh-backup] 后台 GitHub 同步失败：${job.error}`);
      } finally {
        job.settled = true;
    ...
    return { started: true, snapshot: syncJobSnapshot() };
  }

  /** 后台同步的 promise（测试/关闭流程用；正常路径不等待）。 */
  function backgroundSyncSettled() {
    return syncInFlight ?? Promise.resolve();
  }

  /**
   * 装配来源自检：内存里加载的版本 vs profile 里写的 spec。
   *
   * 宿主代码只在冷启动时装配一次；客户端 bundle 却每次请求都从磁盘读。于是
   * 「profile 已指向新 tag、宿主还跑旧代码」是常态（2026-09-10 卡死事故的
   * 直接放大因素：用户以为在跑修好的版本，实际跑的是没有超时的旧版）。
   * 这里把两者如实摊开，任何"方法不存在/行为不对"的现场第一眼就能定性。
   */
  function pluginIdentity() {
    const id = { name: null, version: null, path: fileURLToPath(import.meta.url), profile: null, spec: null, versionMatchesSpec: null, note: null };
    try {
      const pkg = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8'));
      id.name = typeof pkg.name === 'string' ? pkg.name : null;
      id.version = typeof pkg.version === 'string' ? pkg.version : null;
    } catch { /* 读不到就留空 */ }
    ...
    try {
      ...
      for (const entry of readdirSync(profilesDir, { withFileTypes: true })) {
        ...
        let spec = null;
        try {
          spec = JSON.parse(readFileSync(join(profilesDir, entry.name, 'package.json'), 'utf8'))?.dependencies?.[id.name] ?? null;
        } catch { continue; }
        if (!spec) continue;
        id.profile = entry.name;
        id.spec = spec;
        const tag = /#([^#]+)$/.exec(String(spec))?.[1] ?? null;
        if (tag && id.version) {
          id.versionMatchesSpec = tag.replace(/^v/, '') === id.version;
          if (!id.versionMatchesSpec) {
            id.note = `profile「${entry.name}」里写的是 ${tag}，内存中加载的却是 v${id.version} —— 宿主还没冷启动，跑的是旧代码；改完依赖必须重启 dsh 才生效`;
          }
        }
        break;
      }
    } catch { /* 布局不同/无权限：自检降级为沉默 */ }
    return id;
  }

  /**
   * 硬截止时间护栏：把任意 promise 变成“到点必返回”的结果，用于命令/工具出口。
   * 超时不等于取消底层工作（本地 tar/推送会继续跑完并落在状态里），但**命令一定会
   * 结算**——这正是 DSH 命令卡不卡死的充要条件。返回 { timedOut, value, error }。
   */
  async function withDeadline(promise, ms, label) {
    let timer;
    const timeout = new Promise((resolve) => {
      timer = setTimeout(() => resolve(SYNC_DEADLINE), ms);
    });
    try {
      const raced = await Promise.race([promise.then((value) => ({ value }), (error) => ({ error })), timeout]);
      if (raced === SYNC_DEADLINE) {
        return { timedOut: true, error: new Error(`${label}超过 ${Math.round(ms / 1000)} 秒仍未完成`) };
      }
      return { timedOut: false, ...raced };
    } finally {
      clearTimeout(timer);
    }
  }

  /** 命令/工具出口的硬截止时间（毫秒）：默认 180s，可用设置项/环境变量收紧放宽。 */
  function commandDeadlineMs() {
    const env = Number(process.env.DSH_BACKUP_CMD_DEADLINE_SEC);
    const cfgSec = Number(resolvedConfig()?.commandDeadlineSec);
    const sec = Number.isFinite(env) && env >= 5
      ? env
      : (Number.isFinite(cfgSec) && cfgSec >= 5 ? cfgSec : DEFAULTS.commandDeadlineSec);
    return Math.min(3600, Math.max(5, Math.floor(sec))) * 1000;
  }

  /** 可被取消/中止打断的退避睡眠：250ms 粒度轮询，取消后最多 250ms 内生效。 */
  async function sleepInterruptible(ms, signal) {
    const until = Date.now() + Math.max(0, ms);
    while (Date.now() < until) {
      if (signal?.aborted || netCancelRequested) return;
      await new Promise((r) => setTimeout(r, Math.min(250, until - Date.now())));
    }
  }

  /** 生效的“刚性完成”窗口（毫秒）：窗口内无限重试直到成功；0 = 关闭重试。 */
  function netRetryWindowMs() {
    const cfgSec = Number(resolvedConfig()?.githubRetryWindowSec);
    const sec = Number.isFinite(cfgSec) && cfgSec >= 0 ? cfgSec : DEFAULTS.githubRetryWindowSec;
    return Math.max(0, Math.min(86400, Math.floor(sec))) * 1000;
  }

  /** 慢链路友好的 git 参数：默认低速率判定会让“慢而不死”的连接提前失败。 */
  function gitSlowLinkArgs() {
    return ['-c', 'http.lowSpeedLimit=1000', '-c', 'http.lowSpeedTime=300'];
  }

  /**
   * 刚性完成一个网络动作：窗口内重试到成功为止（issue laituli/dsh-personal#2）。
   * 退避 1s→2s→4s→…→30s 封顶；每次尝试跑满 netTimeoutMs()。
   *
   * 长时间等待必须对用户可见：每次尝试/退避都把进度写进 netProgress，面板
   * 「立即同步」按钮旁据此显示“第 N 次 / 已用 Xs / 窗口 Ys / 下次 Zs”，并提供
   * 「取消本次同步」——用户主动中止不算策略降级，脚本化调用仍保持刚性语义。
   */
  /**
   * 确定性网络错误（重试不可能改变结果）：远端不存在、ref/分支不存在、认证被拒……
   * 这类错误绝不进入刚性重试窗口——否则"仓库地址写错"要等 30 分钟才报错
   * （实测：`couldn't find remote ref main` 连试 11 次，把 /backup github pull
   * 一路拖到命令硬截止时间）。只匹配明确的确定性措辞；传输类错误
   * （timeout / connection reset / failed to connect）继续按刚性策略重试。
   */
  const FATAL_NET_PATTERN = /couldn't find remote ref|Repository not found|repository .* not found|could not read Username|Authentication failed|Permission denied|does not appear to be a git repository|not a valid object name|src refspec .* does not match/i;

  async function runNetworkStrict(label, run, signal) {
    const windowMs = netRetryWindowMs();
    const startedAt = Date.now();
    const base = { label, startedAt, windowMs, lastError: null };
    let attempt = 0;
    for (;;) {
      netProgress = { ...base, attempt, elapsedMs: Date.now() - startedAt, nextDelayMs: 0, at: new Date().toISOString() };
      try {
        await run(attempt);
        if (attempt > 0) console.warn(`[dsh-backup] ${label} 第 ${attempt + 1} 次尝试成功（网络恢复）`);
        netProgress = null;
        return;
      } catch (err) {
        if (signal?.aborted) { netProgress = null; throw err; }
        if (netCancelRequested) {
          netProgress = null;
          throw new Error(`已取消：${label}（用户中止本次同步；未做任何降级，可随时重试）`);
        }
        const message = String(err && err.message ? err.message : err);
        if (FATAL_NET_PATTERN.test(message)) {
          netProgress = null;
          throw new Error(`${message}（这是确定性错误——重试不会改变结果，已立即停止重试）`);
        }
        attempt += 1;
        const elapsed = Date.now() - startedAt;
        if (windowMs === 0 || elapsed >= windowMs) { netProgress = null; throw err; }
        const delayMs = Math.min(30000, 1000 * Math.pow(2, Math.min(5, attempt - 1)));
        netProgress = { ...base, attempt, elapsedMs: elapsed, nextDelayMs: delayMs, lastError: message.slice(0, 200), at: new Date().toISOString() };
        console.warn(`[dsh-backup] ${label} 第 ${attempt} 次失败（已用 ${Math.round(elapsed / 1000)}s / 窗口 ${Math.round(windowMs / 1000)}s）：${message.slice(0, 160)} —— 不退让，${Math.round(delayMs / 1000)}s 后继续`);
        await sleepInterruptible(delayMs, signal);
      }
    }
  }

  /** 宿主 Web 监听端口（尽力探测：webServer 服务 → DSH_WEB_URL → 3080）。 */
  function hostWebPort() {
    try {
      const svc = ctx.get('webServer');
      if (svc && Number.isFinite(Number(svc.port)) && Number(svc.port) > 0) return Number(svc.port);
    } catch { /* 无 webServer 服务 */ }
    try {
      const m = /:(\d+)\/?$/.exec(String(process.env.DSH_WEB_URL || ''));
  ...
  /** shell 引号：含空白/引号时用双引号包裹，保证“任意终端可粘贴”。 */
  ...
  /**
   * 还原启动当前宿主的命令（本进程的 node + 脚本 + 原参数）。恢复完成后
   * 需要“停止当前 agent 并启动新的”，这条命令正是原样重启入口。
   * Windows 上以引号开头的可执行文件在 PowerShell 里必须用 `&` 调用，
   * 故 win32 前缀 `& `（cmd.exe 用户去掉行首 & 即可，提示里已注明）。
   * 路径/端口/profile 全部来自本进程与宿主探测，换机器/换用户自动适配。
   */
  function hostRelaunchCommand() {
    const base = [shellQuote(process.execPath), ...process.argv.slice(1).map((a) => shellQuote(a))].join(' ');
    return IS_WIN ? `& ${base}` : base;
  }

  /**
   * 运维脚本（fork 增量）：stop / deploy-restore 都走这里，与上游救援通道分离，
   * 跟随上游合并时零冲突。同样优先用备份目录里的副本（随备份落盘），缺失才回退插件自带。
   */
  function preferredOpsPath() {
    try {
      const { root } = paths();
      const copy = `${root}/ops.mjs`;
      if (readFileSync(copy, 'utf8').length > 0) return copy;
    } catch { /* 备份目录尚无副本 */ }
    return fileURLToPath(new URL('../rescue/ops.mjs', import.meta.url));
  }

  /**
   * 优先用「备份目录里的 rescue 副本」（每次备份刷新、自包含，灾时 profile/插件
   * 可能已损坏时仍可用），缺失才回退插件自带的那份。
   */
  function preferredRescuePath() {
    try {
      const { root } = paths();
      const copy = `${root}/rescue.mjs`;
      if (readFileSync(copy, 'utf8').length > 0) return copy;
    } catch { /* 备份目录尚无副本 */ }
    return fileURLToPath(new URL('../rescue/rescue.mjs', import.meta.url));
  }

  /**
   * 「重启 / 升级」交接指令：正规重启（装/升级插件、升级 dsh、改配置生效）与恢复
   * 后交接共用同一对原语——先停旧宿主，再启新宿主。面板把它显示在备份页里，
   * 让“重启前后各备份一次”成为可照做的四步，而不是靠记忆。
   * - stopCmd：`rescue stop` 按 web 端口找并结束占用进程、等端口释放；宿主不在时幂等跳过。
   * - relaunchCmd：本进程原样启动命令（含 profile 等原参数），任何终端可粘贴。
   */
  /**
   * 非默认环境/工作目录时的前置指令行。宿主每次启动重新解析 DSH_HOME，且会话
   * 分桶依赖 cwd——用自定义 DSH_HOME 或非默认 cwd 启动的宿主，重启指令必须把
   * 这两样带上，否则新宿主会去读默认 ~/.dsh，看起来像“恢复完什么都没了”。
   * Windows 走 PowerShell 方言（$env: / Set-Location），其余走 POSIX。
   */
  function envPrefixLines() {
    const lines = [];
    try {
      const env = ctx.get('launchEnvironment');
      const dshHomeEnv = env?.get('DSH_HOME')?.value;
      const homeRaw = env?.get('HOME')?.value || env?.get('USERPROFILE')?.value;
      const norm = (p) => String(p ?? '').split('\\').join('/').replace(/\/+$/, '');
      if (dshHomeEnv && (!homeRaw || norm(dshHomeEnv) !== `${norm(homeRaw)}/.dsh`)) {
        lines.push(IS_WIN ? `$env:DSH_HOME='${dshHomeEnv}'` : `export DSH_HOME='${dshHomeEnv}'`);
      }
    } catch { /* launchEnvironment 缺失 */ }
    try {
      const cwd = process.cwd();
      if (cwd) lines.push(IS_WIN ? `Set-Location '${cwd}'` : `cd '${cwd}'`);
    } catch { /* 忽略 */ }
    return lines;
  }

  /**
   * 把依赖 spec 规范成“跨机器可解析的安装引用”：
   *   codeload tarball → https://github.com/<owner>/<repo>.git#<ref>
   *   github/git URL 原样；npm 包保留 name@range。
   */
  function normalizeInstallRef(spec) {
    const raw = String(spec ?? '').trim();
    const codeload = /^https?:\/\/codeload\.github\.com\/([^/]+)\/([^/]+)\/tar\.gz\/(.+)$/.exec(raw);
    if (codeload) return `https://github.com/${codeload[1]}/${codeload[2]}.git#${decodeURIComponent(codeload[3])}`;
    return raw;
  }

  /** 当前 dsh 本体版本（尽力探测：本进程 argv[1] 指向 CLI 的 bin.js）。 */
  function dshCliVersion() {
    try {
      const bin = process.argv[1] || '';
      const pkg = `${dirname(dirname(bin))}/package.json`;
  ...
  /**
   * 「从 0 安装」清单：dsh 本体版本 + 每个 profile 的 bundles/依赖（规范为 URL#tag 形态）。
   * 供运维→重装/安装页生成可复制指令 —— 任意机器按这串指令即可复现当前环境。
   */
  /** git 源依赖的 spec 判定（个人 fork 用 github:owner/repo#tag 或 github URL 安装）。 */
  const GIT_SOURCE_SPEC = /^(?:github:|git\+|git:|https?:\/\/github\.com)/;

  /** spec → 可 fetch 的仓库 URL（github:owner/repo → https://github.com/owner/repo.git）。 */
  function repoUrlOf(spec) {
    const s = String(spec || '').trim();
    const gh = /^github:([^#]+)/.exec(s);
    if (gh) return `https://github.com/${gh[1].replace(/\.git$/, '')}.git`;
    const url = /^(?:git\+)?(https?:\/\/[^#]+)/.exec(s);
    return url ? url[1] : null;
  }

  /** spec → 当前 ref（'#' 之后的 tag/sha；没有则 null）。 */
  function refOf(spec) {
    const i = String(spec || '').indexOf('#');
    return i >= 0 ? String(spec).slice(i + 1) : null;
  }

  /** 仅认 vX.Y.Z / X.Y.Z（忽略预发布），返回最大者；无合法 tag 返回 null。 */
  function bestSemverTag(list) {
    const parse = (t) => {
    ...
    for (const t of list) {
      const v = parse(t);
      if (!v) continue;
      const newer = !bestV || v[0] > bestV[0]
        || (v[0] === bestV[0] && (v[1] > bestV[1] || (v[1] === bestV[1] && v[2] > bestV[2])));
      if (newer) { best = t; bestV = v; }
    }
    return best;
  }

  /** 缓存的 git 可执行文件解析（subprocess 服务优先，回退 PATH 上的 git）。 */
  let gitBinCache = null;
  async function gitBin() {
    if (gitBinCache) return gitBinCache;
    try {
      const s = ctx.get('subprocess');
      if (s && typeof s.resolveExecutable === 'function') { gitBinCache = await s.resolveExecutable('git'); return gitBinCache; }
    } catch { /* 无 subprocess 服务：回退 PATH */ }
    gitBinCache = 'git';
    return gitBinCache;
  }

  /**
   * 列出 profile 里所有 git 源依赖，并尽力查远端最新语义化 tag（只读、有界超时）。
   * 网络不通时该条 latest=null + reason，绝不阻塞面板其它部分。
   * 用途：运维→升级页签的「前端内升级」清单（个人 fork 靠 url#tag 升级，
   * 市场那套按钮只跟 npm 版本，跟不到 fork 的 tag）。
   */
  /** 前端内升级任务的实时状态（与 netProgress 一样供面板显示进度/取消）。 */
  let upgradeJob = null;

  /**
   * 把成对停机/启动指令落到备份根：升级会让磁盘上的前端包立刻变新而内存宿主仍旧，
   * 面板本身可能打不开——那时界面上复制不到重启指令，所以必须**升级前**落盘。
   */
  /**
   * 用**显式环境变量**跑一条命令并收集输出。存在的理由：ctx.subprocess.spawn 不接收
   * env，子进程拿到的是宿主记录的那份环境——实测把「隔离验证」的安装动作打到了用户
   * 真实 profile。升级这类必须作用在指定 DSH_HOME 上的动作，不能靠继承环境。
   */
  async function runPlain(argv, cwd, env, timeoutMs) {
    ...
    return await new Promise((resolve, reject) => {
      ...
      const timer = timeoutMs ? setTimeout(() => { try { child.kill(); } catch { /* 已退出 */ } }, timeoutMs) : null;
      child.once('error', (e) => { if (timer) clearTimeout(timer); reject(e); });
      child.once('exit', (code) => { if (timer) clearTimeout(timer); resolve({ out, err, code }); });
    });
  }
  async function writeRestartFile(note) {
    const { root } = paths();
    const plan = restartPlan();
    const text = [
      '★ 重启指令（升级/恢复之后用；界面打不开时照抄这份文件）',
      `生成时间: ${new Date().toISOString()}`,
      note ? `触发场景: ${note}` : null,
      `Web 端口: ${plan.webPort}`,
      '',
      '# 1) 停旧宿主（按端口结束进程，不引用任何本机路径）',
      plan.portableStopCmd,
      '',
      '# 2) 启新宿主（dsh 在 PATH 上）',
      plan.portableRelaunchCmd,
      '',
      '# 3) 用新宿主终端打印的带 token 地址重新打开页面（旧页面的 cookie 属于上一个进程，重启即失效）',
      '',
      '# 本机精确停法（备份目录里的 ops.mjs 副本；只有本机备份过才存在）',
      plan.stopCmd,
      plan.relaunchCmd,
      '',
    ].filter((l) => l !== null).join('\n');
    await writeOwned(`${root}/RESTART.txt`, text);
    return `${root}/RESTART.txt`;
  }

  /** spec → 归一化安装串：github:<owner>/<repo>#<tag>（个人 fork 的跨机安装规范）。 */
  function specForUpgrade(entry, to) {
    const repo = String((entry && entry.repo) || '');
    const gh = /^https?:\/\/github\.com\/(.+?)(?:\.git)?$/.exec(repo);
    if (gh) return `github:${gh[1]}#${to}`;
    return `${entry && entry.name ? entry.name : repo}@${to}`;
  }

  /** 调隔离装配门（rescue/gate-boot.mjs）：隔离 home 里装 spec 并真启动一次宿主。 */
  async function gateBoot(spec, signal) {
    const script = fileURLToPath(new URL('../rescue/gate-boot.mjs', import.meta.url));
    const budgetSec = Math.max(120, Math.round(netTimeoutMs() / 1000) + 120);
    const r = await spawnRun(
      [process.execPath, script, '--spec', spec, '--json', '--exit-zero', '--timeout', String(budgetSec)],
      paths().dshHome, signal, (budgetSec + 60) * 1000,
    );
    for (const line of String(r.out || '').trim().split('\n').reverse()) {
      try { return JSON.parse(line); } catch { /* 非 JSON 行 */ }
    }
    return { ok: false, reason: `装配门输出无法解析: ${String(r.out || r.err || '').slice(0, 200)}` };
  }

  /**
   * 前端内升级事务（单插件/一键全部）：
   * ① 备份一次 → ② RESTART.txt 落盘 → ③ 逐插件过装配门 → ④ 通过才写真实 profile。
   * 全程阶段进度写进 upgradeJob，供面板显示与取消；失败保留原 spec（逐插件独立事务）。
   */
  async function runUpgrade(kind, name, signal) {
    const { dshHome } = paths();
    const plan = await upgradePlanSnapshot(signal);
    const pool = Array.isArray(plan.plugins) ? plan.plugins : [];
    const targets = kind === 'all'
      ? pool.filter((x) => x.hasUpdate)
      : pool.filter((x) => x.name === name && x.hasUpdate);
    if (!targets.length) {
      return {
        ok: false,
        summary: kind === 'all'
          ? '没有可升级的 git 源插件（都已是远端最新 tag，或清单读取失败）'
          : `未找到可升级的插件 ${name}（没有更新的 tag，或它不是 git 源安装）`,
      };
    }
    ...
    const job = {
      ...
      plugins: targets.map((x) => ({ name: x.name, profile: x.profile, from: x.current, to: x.latest, phase: 'pending', ok: null, reason: null })),
    };
    upgradeJob = job;
    try {
      const snap = await doBackup(undefined, signal, { namePrefix: 'dsh-pre-upgrade-' });
      job.backupName = snap.name;
      job.phase = 'restart-file';
      job.restartFile = await writeRestartFile(kind === 'all' ? '一键全部升级（升级前落盘）' : `升级 ${name}（升级前落盘）`);
      for (const item of job.plugins) {
        if (job.cancelRequested || signal?.aborted) { item.phase = 'cancelled'; item.ok = false; item.reason = '用户取消'; continue; }
        const src = targets.find((x) => x.name === item.name) || {};
        const spec = specForUpgrade(src, item.to);
        item.spec = spec;
        item.phase = 'gate';
        const gate = await gateBoot(spec, signal);
        if (!gate.ok) { item.phase = 'gate-failed'; item.ok = false; item.reason = String(gate.reason || '装配门未通过').slice(0, 300); continue; }
        item.phase = 'install';
        try {
          // 必须把 DSH_HOME 钉在本插件自己的数据目录上：spawnRun 只给 cwd，而 dsh CLI
          // 是按环境变量/默认 ~/.dsh 解析 profile 的——不钉就会把插件装进**真实 profile**
          // （实测：隔离验证的安装动作打到了用户真实环境）。
          const prevHome = process.env.DSH_HOME;
          process.env.DSH_HOME = dshHome;
          try {
            const instRes = await runPlain(
              [process.execPath, dshBin, 'plugin', '--profile', item.profile, 'add', spec],
              `${dshHome}/profiles/${item.profile}`,
              { ...process.env, DSH_HOME: dshHome },
              600000,
            );
            if (instRes.code !== 0) throw new Error(`安装失败 exit=${instRes.code}：${String(instRes.err || instRes.out).slice(-300)}`);
            item.stdout = String((instRes && instRes.out) || '').slice(-1200);
            item.stderr = String((instRes && instRes.err) || '').slice(-1200);
            // 效果校验：pnpm/CLI 可能 exit 0 却什么都没改（实测隔离 profile 的 package.json
            // 未变、node_modules/.pnpm 为空）。不校验就会把“假成功”记成 installed。
            const afterPkg = JSON.parse(await readFile(`${dshHome}/profiles/${item.profile}/package.json`, 'utf8'));
            const afterSpec = String((afterPkg.dependencies ?? {})[item.name] ?? '');
            if (!afterSpec.includes(String(item.to))) {
              item.phase = 'install-noop';
              item.ok = false;
              item.reason = `安装命令 exit 0 但 spec 未变成含 ${item.to}（实际: ${afterSpec || '缺失'}）｜输出尾部: ${(item.stdout || item.stderr || '（无输出）').slice(-300)}`;
              continue;
            }
          } finally {
          ...
          item.phase = 'install-failed';
          item.ok = false;
          item.reason = String(err && err.message ? err.message : err).slice(0, 300);
        }
      }
      const done = job.plugins.filter((x) => x.ok).map((x) => `${x.name} → ${x.to}`);
      const failed = job.plugins.filter((x) => x.ok === false).map((x) => `${x.name}: ${x.reason}`);
      job.phase = done.length ? 'restart-pending' : 'failed';
      return {
        ok: done.length > 0,
        summary: [
          `升级事务完成（${job.kind === 'all' ? '一键全部' : '单插件'}）：成功 ${done.length}/${job.plugins.length}`,
          done.length ? `已写入 profile（重启后生效）：${done.join('、')}` : null,
          failed.length ? `未完成：${failed.join('；')}` : null,
          `升级前快照：${job.backupName}`,
          `重启指令已落盘：${job.restartFile}`,
          '现在只差冷启动：按 RESTART.txt 里的成对指令停旧启新（重启前不要刷新旧页面）。',
        ].filter(Boolean).join('\n'),
        backupName: job.backupName,
        restartFile: job.restartFile,
        plugins: job.plugins,
      };
    } finally {
  ...
  async function upgradePlanSnapshot(signal) {
    const { dshHome } = paths();
    const plugins = [];
    let profiles = [];
    try {
      profiles = (await readdir(`${dshHome}/profiles`, { withFileTypes: true })).filter((d) => d.isDirectory()).map((d) => d.name);
    } catch { profiles = []; }
    const git = await gitBin();
    for (const profile of profiles) {
      let pkg = null;
      try { pkg = JSON.parse(await readFile(`${dshHome}/profiles/${profile}/package.json`, 'utf8')); } catch { continue; }
      for (const [name, specRaw] of Object.entries(pkg.dependencies ?? {})) {
        const spec = String(specRaw);
        if (!GIT_SOURCE_SPEC.test(spec)) continue;
        const repo = repoUrlOf(spec);
        const current = refOf(spec);
        const entry = { profile, name, spec, repo, current, latest: null, hasUpdate: false, tagCount: 0, reason: null };
        if (!repo) { entry.reason = '无法从 spec 解析仓库地址'; plugins.push(entry); continue; }
        try {
          // 只读查询加有界重试：GitHub 抖动时不该把「有没有新版」判成“没有”，
          // 但面板上的清单查询又必须**快速**返回（用户在点按钮等结果）。
          // 取渐进超时：第一次 15s（网络好就立刻有结论），第二次 30s（慢链路也能成），
          // 都受 netTimeoutMs() 上限约束；全败只记 reason，绝不拖成"点了没反应"。
          let r = null;
          let lastErr = null;
          const cap = netTimeoutMs();
          const budgets = [Math.min(cap, 15000), Math.min(cap, 30000)];
          for (let attempt = 0; attempt < budgets.length && r === null; attempt += 1) {
            try { r = await spawnRun([git, ...gitSlowLinkArgs(), 'ls-remote', '--tags', '--refs', repo], dshHome, signal, budgets[attempt]); }
            catch (e) { lastErr = e; if (attempt + 1 < budgets.length) await sleepMs(1000 * (attempt + 1)); }
          }
          if (r === null) throw (lastErr ?? new Error('ls-remote 未返回结果'));
          const tags = r.out.split('\n').map((l) => l.trim().split(/\s+/)[1]).filter(Boolean).map((t) => t.replace('refs/tags/', ''));
          entry.latest = bestSemverTag(tags);
          entry.tagCount = tags.length;
          entry.hasUpdate = Boolean(entry.latest && entry.current && entry.latest !== entry.current);
        } catch (err) {
          if (signal?.aborted) throw err;
          entry.reason = networkReason(String(err && err.message ? err.message : err));
        }
        plugins.push(entry);
      }
    }
    return { ok: true, plugins, checkedAt: new Date().toISOString() };
  }
  async function installPlanSnapshot() {
    const { dshHome } = paths();
    const plan = { dshVersion: dshCliVersion(), profiles: [] };
    let names = [];
    try { names = (await readdir(`${dshHome}/profiles`, { withFileTypes: true })).filter((d) => d.isDirectory()).map((d) => d.name); } catch { names = []; }
    for (const name of names) {
      let pkg = null;
      try { pkg = JSON.parse(await readFile(`${dshHome}/profiles/${name}/package.json`, 'utf8')); } catch { continue; }
      const deps = Object.entries(pkg.dependencies ?? {}).map(([dep, spec]) => ({ name: dep, spec: String(spec), ref: normalizeInstallRef(spec) }));
      const bundles = Array.isArray(pkg.dsh?.profile?.bundles) ? pkg.dsh.profile.bundles : [];
      plan.profiles.push({ name, bundles, deps });
    }
    return plan;
  }

  /**
   * 跨机通用的「停旧宿主」：按监听端口结束进程。
   * 不引用任何本机路径——备份目录里的 ops.mjs 只有本机备份过才存在，换机器冷启动时
   * 那条路径根本不成立，所以这是重装/换机流程里唯一可以照抄的停止方式。
   */
  function portableStopCmd(port) {
    return IS_WIN
      ? `$p = (Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty OwningProcess); if ($p) { Stop-Process -Id $p -Force }`
      : `lsof -ti tcp:${port} | xargs -r kill`;
  }

  /** 跨机通用的「启新宿主」：dsh 在 PATH 上（DSH_HOME 非默认时才补环境行）。 */
  function portableRelaunchCmd() {
    const prefix = envPrefixLines().filter((line) => !/^(Set-Location|cd )/.test(line));
    return prefix.length ? `${prefix.join('\n')}\ndsh web` : 'dsh web';
  }

  function restartPlan() {
    const rescuePath = preferredOpsPath();
    const port = hostWebPort();
    const prefix = envPrefixLines();
    // stop 只需要 DSH_HOME（rescue 也按它解析数据目录）；启动新宿主还要回到原 cwd。
    const withPrefix = (cmd, withCwd) => {
      const lines = withCwd ? prefix : prefix.filter((line) => !/^(Set-Location|cd )/.test(line));
      return lines.length ? `${lines.join('\n')}\n${cmd}` : cmd;
    };
    return {
      webPort: port,
      stopCmd: withPrefix(`node ${shellQuote(rescuePath)} stop --web-port ${port}`, false),
      relaunchCmd: withPrefix(hostRelaunchCommand(), true),
      // 换机/冷启动可照抄的那两条（不引用任何本机路径）
      portableStopCmd: portableStopCmd(port),
      portableRelaunchCmd: portableRelaunchCmd(),
    };
  }

  function restoreHandoff({ archiveName, deploy = false, delaySec = undefined }) {
    const { stopCmd, relaunchCmd, webPort } = restartPlan();
    if (!deploy) return { stopCmd, relaunchCmd, offlineCmd: null };
    const { root } = paths();
    const rescuePath = preferredOpsPath();
    const sel = archiveName ? String(archiveName) : 'latest';
    const args = ['deploy-restore', sel, '--yes', '--root', shellQuote(root)];
    if (Number.isFinite(Number(delaySec))) args.push('--delay', String(Math.floor(Number(delaySec))));
    args.push('--web-port', String(webPort));
    return { stopCmd, relaunchCmd, offlineCmd: ['node', shellQuote(rescuePath), ...args].join(' ') };
  }

  /** 上一次武装留下的空闲门控清理函数（重复武装时先摘旧监听）。 */
  let lastDeployGateDispose = null;

  /**
   * 武装一次「部署期恢复」：
   * 1) 派生 detached 孤儿进程（rescue deploy-restore --yes，--arm-file 门控 + 中止文件）；
   * 2) 宿主内监听 agent 事件，等「全部聊天轮次停止」（主会话 agent + 子代理均 idle）后
   *    写入 arm 文件；
   * 3) 执行器随后按设置项延时 → 停止宿主 → 删除(快照后挪旁) → 从备份整体恢复。
   * 本调用立即返回凭据；真实动作全部发生在宿主空闲之后（可删除 abortFile 取消）。
   * 注：恢复动作绝不经 ctx.subprocess（宿主退出钩子会杀子树），故用 node:child_process
   * detached 孤儿进程承载（侦察结论 B/C）。
   */
  function armDeployRestore({ selector, delay, archiveName }) {
    const { root } = paths();
    const delaySec = deployRestoreDelaySec(delay);
    const token = randomUUID();
    const armFile = join(root, `deploy-restore-${token}.arm`);
    const abortFile = join(root, `deploy-restore-${token}.abort`);
    // 清理历史残留标记（避免旧 arm/abort 文件误触发/误取消）
    (async () => {
      try {
        for (const name of await readdir(root)) {
          if (/^deploy-restore-.*\.(arm|abort)$/.test(name)) await rm(join(root, name), { force: true });
        }
      } catch { /* 备份目录不可读则忽略 */ }
    })();

    // 1) 派生外部执行器（宿主死后继续存活）——用 fork 的 ops.mjs 做停机窗口编排
    const rescuePath = preferredOpsPath();
    const port = hostWebPort();
    const pid = process.pid;
    let childError = false;
    const child = spawn(process.execPath, [
      rescuePath, 'deploy-restore', selector || 'latest', '--yes',
      '--root', root,
      '--arm-file', armFile,
    ...
    // 2) 空闲门控：全部 live agent 均非 running → 写 arm 文件
    const disposers = [];
    let armed = false;
    const tryArm = async () => {
      if (armed || childError) return;
      let running = 0;
      try {
        const agentsSvc = ctx.get('agents');
        if (agentsSvc && typeof agentsSvc.list === 'function') {
          for (const a of agentsSvc.list()) if (a && a.status === 'running') running += 1;
        }
      } catch { /* agents 服务缺失 → 按兜底路径处理 */ }
      if (running === 0) {
        armed = true;
        for (const d of disposers) { try { d(); } catch { /* 忽略 */ } }
        disposers.length = 0;
        try {
          await writeFile(armFile, String(Date.now()), 'utf8');
        } catch { /* 写失败由执行器 --arm-timeout 兜底（exit 4） */ }
      }
    };
    if (typeof ctx.on === 'function') {
      disposers.push(ctx.on('agent/status', () => { void tryArm(); }));
      disposers.push(ctx.on('agent/created', () => { void tryArm(); }));
      disposers.push(ctx.on('agent/disposed', () => { void tryArm(); }));
    }
    // agents 服务缺失时的兜底：短延时后直接 arm（执行器自带 delay 缓冲，宿主仍在运行）
    let agentsSvc = undefined;
    try { agentsSvc = ctx.get('agents'); } catch { agentsSvc = undefined; }
    if (!agentsSvc || typeof agentsSvc.list !== 'function') {
      disposers.push(ctx.timeout(() => { void tryArm(); }, 3000));
    }
    void tryArm();

    // 3) 替换上一次武装的门控监听
    if (lastDeployGateDispose) { try { lastDeployGateDispose(); } catch { /* 忽略 */ } }
    lastDeployGateDispose = () => {
      for (const d of disposers) { try { d(); } catch { /* 忽略 */ } }
      disposers.length = 0;
    };
    // 插件卸载时摘监听（宿主存活期间不清理）
    // （apply 生命周期与宿主一致，退出即进程结束，无需额外清理）

    return {
      ok: true,
      deployRestore: true,
      archive: archiveName || selector || 'latest',
      delaySec,
      pid,
      port,
      armFile,
      abortFile,
      summary: `🚀 已武装「部署期恢复」：归档 ${archiveName || selector || 'latest'}。宿主确认无在途聊天轮次后，延时 ${delaySec}s → 停止宿主 → 删除(快照后挪旁) → 从备份整体恢复。\n执行前可随时取消：删除中止文件 ${abortFile}\narm 文件（${armFile}）出现前执行器不会做任何写入。`,
    };
  }
  ...
  const panelOps = {
    /**
     * 面板状态总览：**永不抛错**。任何一步出问题都降级成带 error 字段的最小
     * 快照——「运维界面读不到备份状态」曾经是用户的真实抱怨，状态读失败必须
     * 表现为页面上一行错误说明，而不是整页空白/一直转圈。
     */
    status: async () => {
      const { root, dshHome } = paths();
      try {
        ...
        return {
          ...
          typedBackups,
          // 「重启 / 升级」卡片用的成对指令（停旧宿主 → 启新宿主）；老客户端忽略即可
          restart: restartPlan(),
          installPlan: await installPlanSnapshot(),
          // 前端内升级任务的实时阶段（null = 无任务）
          upgrade: upgradeJob,
          // 刚性重试实时进度（null = 无网络动作在跑）；面板据此在长时间等待里显示进度并提供取消
          net: netProgress,
          // 后台同步任务快照（0.11.13 起网络同步不再阻塞命令，进度看这里）
          sync: syncJobSnapshot(),
        ...
        return {
          ...
          sync: syncJobSnapshot(),
          plugin: pluginIdentity(),
          error: friendlyReason(err),
          errorSummary: `读取备份状态失败：${friendlyReason(err)}（备份文件本身未受影响；可用备份目录里的救援页查看）`,
        };
      }
    },
    ...
    restore: async (selector, dryRun, types, syncDeps, deploy, deployDelay, signal) => {
      try {
        const validTypes = Array.isArray(types) && types.length ? [...new Set(types.filter((t) => BACKUP_TYPES[t]))] : undefined;
        if (Boolean(deploy) && !Boolean(dryRun)) {
          const picked = await pickArchive(selector || 'latest').catch(() => null);
          const arm = armDeployRestore({ selector: selector || 'latest', delay: deployDelay, archiveName: picked ? picked.name : undefined });
          // 部署期恢复：执行器接管“停宿主→恢复”，人只需在完成后重启 dsh；
          // 若执行器异常缺席，offlineCmd 是等价的手动停机恢复指令（任意 shell 可粘贴）。
          const handoff = restoreHandoff({ archiveName: arm.archive, deploy: true, delaySec: arm.delaySec });
          arm.summary += `\n恢复完成后重启 dsh（任何终端，先停旧再启新）:\n  ① 停止旧宿主: ${handoff.stopCmd}\n  ② 启动新宿主: ${handoff.relaunchCmd}${handoff.offlineCmd ? `\n执行器未生效时，可手动停机恢复:\n  ${handoff.offlineCmd}` : ''}`;
          return { ok: true, ...arm, stopCmd: handoff.stopCmd, relaunchCmd: handoff.relaunchCmd, offlineCmd: handoff.offlineCmd };
        }
        const r = await restoreArchive(selector || 'latest', Boolean(dryRun), signal, { syncDeps: Boolean(syncDeps), ...(validTypes ? { types: validTypes } : {}) });
        if (r.dryRun) {
          return { ok: true, dryRun: true, archive: r.archive, files: r.files, sample: r.sample, preflight: r.preflight ?? [], targetExists: r.targetExists, ...(r.merge ? { merge: true, types: r.types, willOverwrite: r.willOverwrite ?? [] } : {}), summary: summarizeRestore(r), stopCmd: null, relaunchCmd: null, offlineCmd: null };
        }
        // 真实恢复（整包或类型 merge）：完成后必须重启 dsh 才能切换生效——
        // stopCmd + relaunchCmd 把“停止旧宿主 → 启动新的”变成可复制的成对指令。
        const handoff2 = restoreHandoff({ archiveName: r.archive, deploy: false });
        return {
          ok: true,
          dryRun: false,
          archive: r.archive,
          ...
          stopCmd: handoff2.stopCmd,
          relaunchCmd: handoff2.relaunchCmd,
          offlineCmd: null,
        };
      } catch (err) {
    ...
    /**
     * 取消进行中的网络动作：只置位标志，让 runNetworkStrict 在当前尝试结束后
     * 停止重试。用户主动中止 ≠ 网络波动降级——不改变默认刚性策略。
     */
    cancelSync: async () => {
      if (!netProgress) {
        return { ok: false, summary: `当前没有进行中的网络动作。\n${syncJobLine()}` };
      }
      netCancelRequested = true;
      return { ok: true, summary: `已请求取消「${netProgress.label}」：当前这次尝试结束后立即停止重试（最多再等 ${Math.round(netTimeoutMs() / 1000)}s）。\n${syncJobLine()}` };
    },
    githubStatus: async () => {
      await adoptRuntimeStateRoot();
      const cfg = githubConfig();
      const { root } = paths();
      ...
      return {
        ...
        // 后台任务快照（0.11.13）：面板顶部的同步进度行读它；老客户端忽略即可
        sync: syncJobSnapshot(),
      };
    },
    /**
     * 面板「立即同步」：**发起后台任务后立刻返回**（不再让 RPC 一挂就是几十分钟）。
     * 进度经 status.sync / status.net 轮询呈现，取消走 cancelSync。
     */
    githubSyncNow: async (signal) => {
      const started = startBackgroundSync('面板立即同步', signal);
      return {
        ok: true,
        background: true,
        ...
        pushed: false,
        tooBig: [],
        sync: started.snapshot ?? null,
      };
    },
    githubPull: async (signal) => {
      netCancelRequested = false;
      try {
        const p = await withWorktreeLock(() => githubPull(signal));
        const lines = [];
    ...
    /**
     * 前端内升级的清单：列出 git 源依赖 + 远端最新语义化 tag（只读）。
     * 真正的升级动作走 upgradeRun（下一步实现）：备份 → 隔离装配门 → pnpm add → 提示冷启动。
     */
    /** 执行升级事务：kind='all' 一键全部；否则 name=单个插件名。 */
    upgradeRun: async (kind, name, signal) => {
      try { return await runUpgrade(kind === 'all' ? 'all' : 'one', name, signal); }
      catch (err) { return { ok: false, summary: friendlyReason(err) }; }
    },
    /** 取消进行中的升级事务（在装配门/安装之间生效，不打断正在跑的 pnpm）。 */
    cancelUpgrade: async () => {
      if (!upgradeJob || upgradeJob.finishedAt) return { ok: false, summary: '当前没有进行中的升级任务。' };
      upgradeJob.cancelRequested = true;
      return { ok: true, summary: '已请求取消：当前这一步结束后停止后续插件（已写入的 spec 不回滚，可用「回滚」重装旧 tag）。' };
    },
    upgradePlan: async (signal) => {
      try { return await upgradePlanSnapshot(signal); }
      catch (err) { return { ok: false, plugins: [], error: friendlyReason(err) }; }
      ...
      handler: async (req, res) => {
        try {
          ...
          try {
            ...
            // 目的地可能刚被改：立刻重读该根的运行时状态（含 GitHub 仓库归属），
            // 否则旧根的自动同步仓库会继续作用于新根
            if (body.reset === true || body.destination !== undefined) {
              // 无条件作废：即使 adopt 的"首次归属"分支不会清状态，也不能让旧根的
              // 运行时 GitHub 仓库（可能带 token）活过目的地切换
              githubState = { repo: null, lastPush: null, lastError: null };
              runtimeStateRoot = null; // 强制重新归属，即使路径字符串相同
              await adoptRuntimeStateRoot();
            }
          } catch (err) {
  ...
  // 卸载/热更新时的收敛：后台同步不再有人 await，卸载时必须主动让正在重试的
  // 网络动作尽快停下（置位取消标志即可；不 await——卸载不该被网络拖住）。
  // 测试进程同理：残留的重试定时器会让 node 一直不退出。
  if (typeof ctx.on === 'function') {
    ctx.on('dispose', () => { if (syncInFlight) netCancelRequested = true; });
  }
  ...
  void (async () => {
    ...
    // 诊断行（永久保留）：任何“方法不存在/版本不对”类问题，第一眼看加载的是哪份代码、
    // panelOps 上到底有没有那些方法——比反复推理快得多。
    const methodProbe = ['status', 'upgradePlan', 'upgradeRun', 'cancelUpgrade']
      .map((k) => k + (typeof panelOps[k] === 'function' ? '+' : '-')).join(' ');
    const ident = pluginIdentity();
    console.log(`[dsh-backup] 装配 v${ident.version ?? '?'} ${ident.path}｜panelOps ${methodProbe}${ident.spec ? `｜profile「${ident.profile}」spec ${ident.spec}` : ''}`);
    // 内存版本 ≠ profile spec：宿主不会因依赖变化而热重载插件代码，必须重启才生效。
    // 这条警告是"改了依赖却没重启，用户以为在跑新代码"这一整类事故的显式信号。
    if (ident.note) console.warn(`[dsh-backup] ⚠️ 版本错配：${ident.note}`);
    const h = await loadAutoState();
    // 记录本次运行时状态的归属根：之后目的地被改时才能识别"换根"并作废旧状态
    runtimeStateRoot = paths().root;
    if (h > 0 && !autoDispose) {
      autoHours = h;
      scheduleAuto();
    }
