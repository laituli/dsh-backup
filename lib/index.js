...
export function apply(ctx, pluginConfig) {
  ...
  async function saveAutoState() {    const { root } = paths();
  ...
  ctx.commands.register({
    name: 'backup',
    description: '备份/恢复 DSH 数据；子命令: list | verify [前缀|all] | restore <前缀|latest> [--dry-run] [--sync-deps] [--deploy] [--deploy-delay N] | doctor [--repair <前缀|latest>] | auto [N小时|off] | github status|sync|pull|repo | [--keep N]',
  ...
  async function runNetworkStrict(label, run, signal) {
    ...
    for (;;) {
      ...
      try {
        ...
        netProgress = { ...base, attempt, elapsedMs: elapsed, nextDelayMs: delayMs, lastError: String(err && err.message ? err.message : err).slice(0, 200), at: new Date().toISOString() };
        console.warn(`[dsh-backup] ${label} 第 ${attempt} 次失败（已用 ${Math.round(elapsed / 1000)}s / 窗口 ${Math.round(windowMs / 1000)}s）：${String(err && err.message ? err.message : err).slice(0, 160)} —— 不退让，${Math.round(delayMs / 1000)}s 后继续`);
    ...
    for (const profile of profiles) {
      ...
      for (const [name, specRaw] of Object.entries(pkg.dependencies ?? {})) {
        ...
        try {
          ...
          // 刻意不用刚性窗口——面板上的清单查询必须快速返回，卡住就只记 reason。
          let r = null;
          let lastErr = null;
          for (let attempt = 1; attempt <= 3 && r === null; attempt += 1) {
            try { r = await spawnRun([git, ...gitSlowLinkArgs(), 'ls-remote', '--tags', '--refs', repo], dshHome, signal, netTimeoutMs()); }
            catch (e) { lastErr = e; if (attempt < 3) await sleepMs(1000 * attempt); }
          }
  ...
  const panelOps = {
     ...
     */
    cancelSync: async () => {
      if (!netProgress) return { ok: false, summary: '当前没有进行中的网络动作（同步可能已经结束）。' };
    ...
    githubSyncNow: async (signal) => {
      ...
      try {
        ...
        // blocked = 远端存在本机未拉取的归档、本次主动不推——不是错误，但面板以
        // ok:false 呈现警示（client githubSyncSchema 只含 ok/summary/pushed/tooBig）
        return { ok: s.blocked ? false : true, summary: summarizeSync(s), pushed: Boolean(s.pushed), tooBig: s.tooBig ?? [] };
      } catch (err) {
        const message = String(err && err.message ? err.message : err);
        githubState = { ...githubState, lastError: message };
        await saveAutoState();
  ...
  void (async () => {
    ...
    console.log(`[dsh-backup] 装配 ${fileURLToPath(import.meta.url)}｜panelOps ${methodProbe}`);
