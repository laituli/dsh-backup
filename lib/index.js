  ...
  async function upgradePlanSnapshot(signal) {
    ...
    for (const profile of profiles) {
      ...
      for (const [name, specRaw] of Object.entries(pkg.dependencies ?? {})) {
        ...
        try {
          // 只读查询加有界重试（2 次退避）：GitHub 抖动时不该把「有没有新版」判成“没有”。
          // 刻意不用刚性窗口——面板上的清单查询必须**快速**返回（用户在点按钮等结果），
          // 卡住就只记 reason。单次超时另取更短的上限：真实实测里 3 次 ×40s 会把
          // 交互式查询拖过 2 分钟，看起来就是"检查更新没反应"。
          let r = null;
          let lastErr = null;
          const perTryMs = Math.min(netTimeoutMs(), 15000);
          for (let attempt = 1; attempt <= 2 && r === null; attempt += 1) {
            try { r = await spawnRun([git, ...gitSlowLinkArgs(), 'ls-remote', '--tags', '--refs', repo], dshHome, signal, perTryMs); }
            catch (e) { lastErr = e; if (attempt < 2) await sleepMs(1000 * attempt); }
          }
