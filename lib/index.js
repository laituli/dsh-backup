...
export function apply(ctx, pluginConfig) {
  ...
  async function githubSync(signal) {
    ...
    // ---- 有变更：先对齐远端（有界网络操作）----
    try {
      await runNetworkStrict('git-fetch', () => spawnRun([git, ...gitSlowLinkArgs(), 'fetch', 'origin'], syncDir, signal, netTimeoutMs()));
    } catch (err) {
    ...
    try {
      await runNetworkStrict('git-push', () => spawnRun([git, ...gitSlowLinkArgs(), 'push', 'origin', 'HEAD:main', '--force-with-lease'], syncDir, signal, netTimeoutMs()));
    } catch (err) {
      if (signal?.aborted) throw err;
      const raw = String(err && err.message ? err.message : err);
  ...
  async function githubPull(signal) {
    ...
    try {
      await runNetworkStrict('git-pull-fetch', () => spawnRun([git, ...gitSlowLinkArgs(), 'fetch', 'origin', 'main'], syncDir, signal, netTimeoutMs()));
    } catch (err) {
      if (signal?.aborted) throw err;
   ...
   */
  async function runNetworkStrict(label, run) {
    ...
    for (;;) {
      try {
        ...
        if (windowMs === 0 || elapsed >= windowMs) throw err;
