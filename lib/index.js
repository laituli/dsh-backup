...
const SETTINGS_FIELDS = ['destination', 'keep', 'exclude', 'redact', 'githubRepo', 'deployRestoreDelay', 'githubTimeoutSec'];
...
export function apply(ctx, pluginConfig) {
  ...
  async function githubSync(signal) {
    ...
    // ---- 有变更：先对齐远端（有界网络操作）----
    try {
      await spawnRun([git, 'fetch', 'origin'], syncDir, signal, netTimeoutMs());
    } catch (err) {
    ...
    try {
      await spawnRun([git, 'push', 'origin', 'HEAD:main', '--force-with-lease'], syncDir, signal, netTimeoutMs());
    } catch (err) {
      if (signal?.aborted) throw err;
      const raw = String(err && err.message ? err.message : err);
  ...
  async function githubPull(signal) {
    ...
    try {
      await spawnRun([git, 'fetch', 'origin', 'main'], syncDir, signal, netTimeoutMs());
    } catch (err) {
      if (signal?.aborted) throw err;
