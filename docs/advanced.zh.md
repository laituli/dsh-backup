...
- 推送方式是 `HEAD:main --force-with-lease`：远端 main 与本地备份目录保持同步。同步**只管理 `dsh-*` 归档及其边车文件**——新增备份会推上去，本机轮换 / 删除的旧备份会同步移除；仓库里其它文件（如 `README.md`、说明文档）会原样保留、不会被同步删除（v0.11.4+）。因此冷启动指南可以放心放在数据仓库根目录。
- **多机护栏（v0.11.5+）**：删除只作用于本机「上一轮推送过的」备份——如果远端存在本机从未推送过、也尚未拉取的归档（比如另一台机器刚推的），`github sync` 会先停下来提示你「从 GitHub 拉取」，绝不会把这些归档当旧文件删掉。多机共用同一数据仓库时，先拉取再推送即可安全叠加。
- **不空转网络（v0.11.5+）**：本地没有新备份、也没有轮换待同步时，`github sync` / 面板「立即同步」直接返回「无变更」，不再每次先联网 `git fetch`；需要真正推送时 fetch / push 也都有超时保护（约 40 秒），网络不通会明确报错，而不是长时间卡在“同步中…”。
