...
- 推送方式是 `HEAD:main --force-with-lease`：远端 main 与本地备份目录保持同步。同步**只管理 `dsh-*` 归档及其边车文件**——新增备份会推上去，本地轮换 / 删除的旧备份会同步移除；仓库里其它文件（如 `README.md`、说明文档）会原样保留、不会被同步删除（v0.11.4+）。因此冷启动指南可以放心放在数据仓库根目录。
